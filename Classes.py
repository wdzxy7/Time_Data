...
class Test:
    def __init__(self, runtime, cpu=4, ram=8):
    ...
    def insert_data(self, front=0, back=500000):
        back = random.randint(300000, 600000)
        for i in range(1, 12):
    ...
    def test(self, loop_time=10):
        df = DataFrame([], columns=['cpu', 'ram', 'sql', 'data_sum', 'time'])
        for i in range(1, 12):
            print(i)
            for j in range(loop_time):
                ...
                diff = t2 - t1
                arr = DataFrame([[sql, self.data_sum[i], diff]], columns=['cpu', 'ram', 'sql', 'data_sum', 'time'])
